...
"""    def __init__(
    ...
    def generate(
        self, inputs: Union[str, List[str], Image.Image, List[Image.Image]]
    ) -> List[List[float]]:
        ...
        if self.provider == EmbeddingProvider.OPENAI:
            ...
            if isinstance(inputs[0], str):
                ...
                raise ValueError(
                    "Hugging Face provider does not support image embeddings in this implementation"
                )
            ...
            if isinstance(inputs[0], str):
                ...
                raise ValueError(
                    "SentenceTransformers provider does not support image embeddings"
                )
        elif self.provider == EmbeddingProvider.CLIP:
            ...
            if isinstance(inputs[0], str):
                ...
                images = [
                    preprocess(img).unsqueeze(0).to(self.device) for img in inputs
                ]
