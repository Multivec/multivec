...
class Qdrant(BaseVectorDB):
        ...
        try:
            ...
            if connection_type == QdrantConnectionType.CLOUD:
                if not url or not api_key:
                    raise ValueError(
                        "URL and API key are required for cloud connection"
                    )
                ...
                self.client = QdrantClient(
                    host=self.host, port=self.port, prefer_grpc=self.prefer_grpc
                )
    ...
    def _prepare_point(self, document: BaseDocument, vector: Vector) -> PointStruct:
        ...
        return PointStruct(id=document.id, vector=vector.data, payload=metadata)
    ...
    def add_documents(
        ...
        if not self.collection_name:
            raise QdrantError(
                "No collection selected. Please create or select a collection first."
            )
        ...
        try:
            points = [
                self._prepare_point(doc, vec) for doc, vec in zip(documents, vectors)
            ]
    ...
    def create_payload_index(
        self, field_name: str, field_schema: Union[str, Dict[str, Any]]
    ) -> None:
        """
